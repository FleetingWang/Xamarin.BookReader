        ...
        private class CustomViewHolder : BaseViewHolder<SearchDetail.SearchBooks>
        {
            ...
            public override void setData(SearchDetail.SearchBooks item)
            {
                holder.setRoundImageUrl(Resource.Id.ivBookCover, Constant.IMG_BASE_URL + item.cover, Resource.Drawable.cover_default)
                        ...
                        .setText(Resource.Id.tvLatelyFollower, String.Format(mContext.GetString(Resource.String.search_result_lately_follower), item.latelyFollower))
                        .setText(Resource.Id.tvRetentionRatio, (TextUtils.IsEmpty(item.retentionRatio) ? String.Format(mContext.GetString(Resource.String.search_result_retention_ratio), "0")
                                : String.Format(mContext.GetString(Resource.String.search_result_retention_ratio), item.retentionRatio)))
                        .setText(Resource.Id.tvBookListAuthor, String.Format(mContext.GetString(Resource.String.search_result_author), item.author));
            }
        }
    }
